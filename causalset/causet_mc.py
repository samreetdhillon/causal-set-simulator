...
def plot_causet(points, R, dim=2, title="Causal Set", show=True, save_path=None, draw_hasse=True):
    ...
    - For dim==2: plot (x,t) with directed Hasse edges (arrowheads)
    - For dim==3: create a 3D scatter (x,y,t) or (t,x,y) and draw Hasse edges as line segments

    Parameters:
        points : (N,2) or (N,3) array-like
        R : reachability matrix (transitive closure)
        show : whether to call plt.show() (set False when saving programmatically)
        save_path : if provided, save figure to this path
        draw_hasse : if True, only draw covering edges; otherwise draw full R edges
    """
